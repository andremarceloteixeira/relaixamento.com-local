...
class DjClassifiedsModelPayments extends JModelList{
	...
	function getPayments(){
		if(empty($this->_items)) {
			...
			$db= JFactory::getDBO();	
			$query = "SELECT p.*, i.name as i_name,pp.name as pp_name, u.name as u_name FROM #__djcf_payments p "
	...
	public function _getListQuery(){
		$query = "SELECT * FROM #__djcf_payments p WHERE 1 ".$this->_buildWhere();
		return $query;
	}
